    ...
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                ...
                _pagination = $.common.isEmpty(options.pagination) ? $.common.visible(options.pagination) : options.pagination;
                _tableObj = $.common.isEmpty(options.tableObj) ? $('#bootstrap-table') : options.tableObj;
                _toolbarId = $.common.isEmpty(options.toolbarId) ? '#toolbar' : options.toolbarId;
                _tableObj.bootstrapTable({// #bootstrap-table
            ...
            serialNumber: function (index) {
                var table = _tableObj.bootstrapTable('getOptions');
            ...
            search: function (formId) {
                ...
                var params = _tableObj.bootstrapTable('getOptions');
                ...
                _tableObj.bootstrapTable('refresh', params);
            },
            // 导出数据
            ...
            // 刷新表格
            refresh: function (Jobj) {
                var JQtable = $.common.isEmpty(Jobj) ? _tableObj : Jobj;
                JQtable.bootstrapTable('refresh', {
                    silent: true
                });
            },
            // 查询表格指定列值
            selectColumns: function (column) {
                return $.map(_tableObj.bootstrapTable('getSelections'), function (row) {
                    return row[column];
                });
            },
            // 查询表格首列值
            ...
            // 回显数据字典(不带样式)
            selectDictLabelNoStyle: function (datas, value) {
                var actions = [];
                $.each(datas, function (index, dict) {
                    if (dict.dictValue == value) {
            ...
            showColumn: function (column) {
                _tableObj.bootstrapTable('showColumn', column);
            },
            // 隐藏表格指定列
            hideColumn: function (column) {
                _tableObj.bootstrapTable('hideColumn', column);
            }
        },
        // 表格树封装处理
        treeTable: {
        ...
        modal: {
            ...
            alert: function (content, type, title) {
                layer.alert(content, {
                    ...
                    title: $.common.isEmpty(title) ? "系统提示" : title,
                    btn: ['确认'],
            ...
            alertError: function (content, title) {
                $.modal.alert(content, modal_status.FAIL, title);
            },
            // 成功提示
            alertSuccess: function (content, title) {
                $.modal.alert(content, modal_status.SUCCESS, title);
            },
            // 警告提示
            alertWarning: function (content, title) {
                $.modal.alert(content, modal_status.WARNING, title);
            },
            // 关闭窗体
            close: function () {
        ...
        http:{
            post: function(url, data, fun, contentType){
                $.http.submit(url, 'POST', data, fun, contentType);
            },
            submit: function (url, type, data, fun, contentType) {
                $.ajax({
                    type: type,
                    url: url,
                    contentType: $.common.isEmpty(contentType) ? "application/json; charset=utf-8" : contentType,
                    data: data,
                    dataType: "json",
